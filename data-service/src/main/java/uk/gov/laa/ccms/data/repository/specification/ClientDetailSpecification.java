 ...
 * client details based on optional parameters such as first name, the clients surname at birth, and
 * date of birth.
 *
...
public final class ClientDetailSpecification {
  ...
  /**
   * Builds a specification to filter {@link ClientDetail} entities based on multiple optional
   * criteria.
   *
   ...
   *     results.
   */
  public static Specification<ClientDetail> filterBy(
    ...
    return (Root<ClientDetail> root, CriteriaQuery<?> query, CriteriaBuilder cb) -> {
      ...
      if (isNotEmpty(surnameAtBirth)) {
        predicate =
            cb.and(
                predicate,
                cb.like(
                    cb.upper(root.get("surnameAtBirth")),
      ...
      if (isNotEmpty(gender)) {
        predicate = cb.and(predicate, cb.equal(cb.upper(root.get("gender")), gender.toUpperCase()));
      }
      if (isNotEmpty(clientReferenceNumber)) {
        predicate =
            cb.and(
                predicate,
                cb.like(
                    cb.upper(cb.toString(root.get("clientReferenceNumber"))),
      ...
      if (isNotEmpty(homeOfficeReference)) {
        predicate =
            cb.and(
                predicate,
                cb.like(
                    cb.upper(root.get("homeOfficeNumber")),
      ...
      if (isNotEmpty(nationalInsuranceNumber)) {
        predicate =
            cb.and(
                predicate,
                cb.like(
                    cb.upper(root.get("nationalInsuranceNumber")),
