 ...
 * notifications based on optional parameters such as case reference number, provider case reference
 * and assigned user ID.
 *
...
public final class NotificationInfoSpecification {
   ...
   * @param includeClosed an optional flag to determine whether to include closed notifications; if
   *     false, only open notifications are included
   ...
   *     this date
   * @param assignedDateTo an optional end date to filter notifications assigned on or before this
   *     date
   * @return a {@link Specification} of {@link NotificationInfo} containing the constructed
   *     filtering predicates
   */
  public static Specification<NotificationInfo> filterBy(
      ...
      final LocalDate assignedDateFrom,
      final LocalDate assignedDateTo) {
      ...
      if (isNotEmpty(caseReferenceNumber)) {
        predicate =
            cb.and(
                predicate,
                cb.like(
                    cb.upper(root.get("lscCaseRefReference")),
      ...
      if (isNotEmpty(providerCaseReference)) {
        predicate =
            cb.and(
                predicate,
                cb.like(
                    cb.upper(root.get("providerCaseReference")),
      ...
      if (isNotEmpty(assignedToUserId)) {
        predicate =
            cb.and(
                predicate,
                cb.like(
                    cb.upper(root.get("assignedTo")), "%" + assignedToUserId.toUpperCase() + "%"));
      }
      if (isNotEmpty(clientSurname)) {
        predicate =
            cb.and(
                predicate,
                cb.like(
                    cb.upper(root.get("personLastName")), "%" + clientSurname.toUpperCase() + "%"));
      }
      ...
      if (isNotEmpty(notificationType)) {
        predicate =
            cb.and(
                predicate,
                cb.equal(
                    cb.upper(root.get("actionNotificationInd")), notificationType.toUpperCase()));
      }
      if (Objects.nonNull(assignedDateFrom)) {
        predicate =
            cb.and(predicate, cb.greaterThanOrEqualTo(root.get("dateAssigned"), assignedDateFrom));
      }
      if (Objects.nonNull(assignedDateTo)) {
        predicate =
            cb.and(predicate, cb.lessThanOrEqualTo(root.get("dateAssigned"), assignedDateTo));
      }
