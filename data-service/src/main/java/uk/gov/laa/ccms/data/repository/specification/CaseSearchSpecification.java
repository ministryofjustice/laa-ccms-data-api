...
/**
 * Utility class for creating a JPA {@link Specification} to query {@link CaseSearch} entities using
 * various filter criteria.
 *
 * <p>The primary method {@code filterBy} allows dynamic construction of predicates for filtering
 * cases based on optional parameters such as case references, provider details, client information,
 * and case statuses.
 *
...
public final class CaseSearchSpecification {
  ...
  /**
   * Builds a specification to filter {@link CaseSearch} entities based on multiple optional
   * criteria.
   *
   ...
   */
  public static Specification<CaseSearch> filterBy(
      final long providerFirmPartyId,
    ...
    return (Root<CaseSearch> root, CriteriaQuery<?> query, CriteriaBuilder cb) -> {
      ...
      if (isNotEmpty(caseReferenceNumber)) {
        predicate = cb.and(predicate, cb.equal(root.get("lscCaseReference"), caseReferenceNumber));
      }
      if (isNotEmpty(providerCaseReference)) {
        predicate =
            cb.and(
                predicate,
                cb.like(root.get("providerCaseReference"), "%" + providerCaseReference + "%"));
      }
      ...
      if (isNotEmpty(clientSurname)) {
        predicate =
            cb.and(
                predicate,
                cb.like(
                    cb.lower(root.get("personLastName")), "%" + clientSurname.toLowerCase() + "%"));
      }
