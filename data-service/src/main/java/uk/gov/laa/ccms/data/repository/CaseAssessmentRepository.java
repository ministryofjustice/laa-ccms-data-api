 ...
 * from the database. It uses {@link JdbcTemplate} to interact with the "XXCCMS_SOA_REPLACE" package
 * to utilize the "GET_ASSESSMENT_DETAILS" function.
 *
 * @see JdbcTemplate
...
public class CaseAssessmentRepository {
  ...
  /**
   * Retrieves a list of case assessment details associated with a specific case reference and
   * assessment type. The details are fetched using a database call to the "XXCCMS_SOA_REPLACE"
   * package, containing the "GET_ASSESSMENT_DETAILS" function.
   *
   * @param caseReference The unique identifier of the case for which assessment details are
   *     required.
  ...
  public List<CaseAssessmentDetail> getCaseAssessmentDetails(
      String caseReference, AssessmentType assessmentType) {
  ...
  private CaseAssessmentDetail mapStructToCaseAssessmentDetail(Struct struct) {
    try {
      ...
      caseAssessmentDetail.setAttributeUserDefinedIndicator(
          Boolean.valueOf((String) struct.getAttributes()[5]));
