 ...
 *
 * This controller provides endpoints for retrieving various types of lookup values.
 * Lookup values are essentially the enumerated values stored in a system for fields that rarely change.
 * The lookup values provide more context and meaning to these fields, by providing the actual
 * human-readable text instead of the stored numerical or coded value.
 *
 * This controller supports operations for three types of lookup values: Common, Amendment Type, and Case Status.
 */
@RestController
...
public class LookupController implements LookupApi {
     ...
     *
     * This endpoint retrieves a paginated list of common lookup values based on the given type and code.
     *
     ...
     *
     * This endpoint retrieves a paginated list of amendment type lookup values based on the given application type.
     *
     * @param applicationType     the application type of the amendment type lookup values
     * @param pageable            pagination information
     * @return the ResponseEntity with status 200 (OK) and the list of amendment type lookup values in the body
     */
    @Override
    public ResponseEntity<AmendmentTypeLookupDetail> getAmendmentTypeLookupValues(String applicationType, Pageable pageable) {
        AmendmentTypeLookupValue example = new AmendmentTypeLookupValue();
        example.setApplicationTypeCode(applicationType);

        Page<AmendmentTypeLookupValue> page = lookupService.getAmendmentTypeLookupValues(Example.of(example), pageable);
     ...
     *
     * This endpoint retrieves a paginated list of case status lookup values based on the given code and copyAllowed flag.
     *
