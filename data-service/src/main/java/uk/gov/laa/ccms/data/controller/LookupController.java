 ...
 *
 * <p>This controller provides endpoints for retrieving various types of lookup values. Lookup
 * values are essentially the enumerated values stored in a system for fields that rarely change.The
 * lookup values provide more context and meaning to these fields, by providing the actual
 * human-readable text instead of the stored numerical or coded value.
 *
 * <p>This controller supports operations for three types of lookup values: Common, Amendment Type,
 * and Case Status.
 */
@RestController
...
public class LookupController implements LookupApi {
   ...
   *
   * <p>This endpoint retrieves a paginated list of common lookup values based on the given type and
   * code.
   *
  ...
  public ResponseEntity<CommonLookupDetail> getCommonLookupValues(
    ...
    return ResponseEntity.ok(
        lookupService.getCommonLookupValues(type, code, description, pageable));
  }

  /**
   * GET lookup values for countries.
   *
   * <p>This endpoint retrieves a paginated list of lookup values for countries
   *
  ...
  public ResponseEntity<CommonLookupDetail> getCountriesLookupValues(
      final String code, final Pageable pageable) {
  ...
  public ResponseEntity<DeclarationLookupDetail> getDeclarations(
      final String type, final String billType, final Pageable pageable) {
   ...
   * given proceeding code, matter type, category of law, and pageable.
   *
   ...
   * description, matter type, category of law, and pageable.
   *
   ...
   * based on the given proceeding code, client involvement type, and pageable.
   *
   * @param proceedingCode the proceeding code
   ...
   * @param pageable pagination information
   * @return the ResponseEntity with status 200 (OK) and the list of amendment type lookup values in
   *     the body
   */
  @Override
   ...
   * @return a ResponseEntity with status 200 (OK) and the list of assessment summary attributes in
   *     the body
   */
  @Override
  public ResponseEntity<AssessmentSummaryEntityLookupDetail> getAssessmentSummaryAttributes(
      final String summaryType, final Pageable pageable) {
   ...
   * @param pageable pagination information
   * @return the ResponseEntity with status 200 (OK) and the list of outcome result values in the
   *     body
   */
  @Override
  public ResponseEntity<OutcomeResultLookupDetail> getOutcomeResultLookupValues(
    ...
    return ResponseEntity.ok(
        lookupService.getOutcomeResultLookupValues(proceedingCode, outcomeResult, pageable));
  }
   ...
   * @param pageable pagination information
   * @return the ResponseEntity with status 200 (OK) and the list of relationships to case result
   *     values in the body
   */
  @Override
  public ResponseEntity<RelationshipToCaseLookupDetail> getPersonToCaseRelationshipLookupValues(
    ...
    return ResponseEntity.ok(
        lookupService.getPersonToCaseRelationshipLookupValues(code, description, pageable));
  }
  ...
  public ResponseEntity<RelationshipToCaseLookupDetail>
      getOrganisationToCaseRelationshipLookupValues(
          String code, String description, Pageable pageable) {
    return ResponseEntity.ok(
        lookupService.getOrganisationToCaseRelationshipLookupValues(code, description, pageable));
  }
  ...
  public ResponseEntity<StageEndLookupDetail> getStageEndLookupValues(
    ...
    return ResponseEntity.ok(
        lookupService.getStageEndLookupValues(proceedingCode, stageEnd, pageable));
  }
  ...
  public ResponseEntity<AwardTypeLookupDetail> getAwardTypeLookupValues(
    ...
    return ResponseEntity.ok(lookupService.getAwardTypeLookupValues(code, awardType, pageable));
  }

  /**
   * GET category of law lookup values by code, matter type description, and copy cost limit value.
   *
   ...
   * @param pageable pagination information
   * @return the ResponseEntity with status 200 (OK) and the list of category of law values in the
   *     body
   */
  @Override
   ...
   * @param pageable pagination information
   * @return the ResponseEntity with status 200 (OK) and the list of evidence document type values
   *     in the body.
   */
  @Override
