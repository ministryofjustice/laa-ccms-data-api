...
public abstract class AbstractEbsDataService {
  ...
  /**
   * Method to dynamically build a custom ExampleMatcher based on the field values of the supplied
   * example object. By default, the ExampleMatcher will perform an exact match on all fields.
   *
   * <p>If a field value starts AND ends with the wildcard character, a 'contains' Matcher will be
   * added for the field. Alternatively, if a field value starts with the wildcard character, an
   * 'endsWith' Matcher will be added. Or if a field value ends with the wildcard character, a
   * 'startsWith' Matcher will be added for that field.
   *
  ...
  protected ExampleMatcher getWildcardMatcher(Object example) {
    ...
    for (Field field : example.getClass().getDeclaredFields()) {
      ...
      try {
        ...
        if (!field.getType().equals(String.class) || field.get(example) == null) {
          continue;
        }
        ...
        if (fieldValue.startsWith(QUERY_PARAM_WILDCARD)
          ...
          exampleMatcher =
              exampleMatcher.withMatcher(field.getName(), GenericPropertyMatchers.contains());
          // Update the field with the wildcard-removed value
          field.set(
              example,
              fieldValue.substring(
                  QUERY_PARAM_WILDCARD.length(),
                  fieldValue.length() - QUERY_PARAM_WILDCARD.length()));
        } else {
          // If the query param has wildcard at start, set the matcher to 'endsWith'
          if (fieldValue.startsWith(QUERY_PARAM_WILDCARD)) {
            exampleMatcher =
                exampleMatcher.withMatcher(field.getName(), GenericPropertyMatchers.endsWith());
            // Update the field with the wildcard-removed value
            field.set(example, fieldValue.substring(QUERY_PARAM_WILDCARD.length()));
          } else if (fieldValue.endsWith(QUERY_PARAM_WILDCARD)) {
            exampleMatcher =
                exampleMatcher.withMatcher(field.getName(), GenericPropertyMatchers.startsWith());
            // Update the field with the wildcard-removed value
            field.set(
                example,
                fieldValue.substring(0, fieldValue.length() - QUERY_PARAM_WILDCARD.length()));
          }
        }
      } catch (IllegalAccessException e) {
        throw new RuntimeException(
            "Failed to access example property: %s".formatted(field.getName()), e);
      }
    }
