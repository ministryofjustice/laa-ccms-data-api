...
/**
 * NotificationSummaryMapper is an interface for mapping notification count data to a notification
 * summary. Utilizes Java streams to process lists of NotificationCount and derive a summary. This
 * interface uses the MapStruct framework for object mapping and is defined as a Spring component.
 *
 * <p>Note: It computes the sum of notifications, actions, and overdue actions from a list of
 * NotificationCount.
 *
...
public interface NotificationSummaryMapper {
  ...
  /**
   * Converts a list of NotificationCount objects into a NotificationSummary object by calculating
   * the sum of different types of notifications.
   *
   * @param notificationCount a list containing instances of NotificationCount, each with a type and
  ...
  default NotificationSummary toNotificationSummary(
    ...
    if (!isNull(notificationCount)) {
      ...
      //  the sum.
      notificationSum =
          notificationCount.stream()
              .filter(IS_NOTIFICATION_TYPE)
              .mapToInt(NotificationCount::getNotificationCount)
              .sum();
      actionSum =
          notificationCount.stream()
    ...
    return new NotificationSummary()
        .notifications(notificationSum)
        .standardActions(actionSum)
