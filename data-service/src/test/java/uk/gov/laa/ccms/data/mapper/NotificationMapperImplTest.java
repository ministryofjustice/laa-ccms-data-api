...
import uk.gov.laa.ccms.data.entity.*;
...
class NotificationMapperImplTest {
  ...
  void shouldMapSingleNote() {
    ...
    NotificationInfo notificationInfo =
        NotificationInfo.builder()
            .notificationId(2L)
            .notes(Arrays.asList(getNote(1L, "Note Text"), getNote(2L, "Second Note Text")))
            .build();
    // When
    Notification notificationResult = mapper.mapToNotification(notificationInfo);
    // Then
  ...
  void shouldMapDocuments() {
    ...
    NotificationInfo notificationInfo =
        NotificationInfo.builder()
            .notificationId(2L)
            .documents(Arrays.asList(document, documentTwo))
            .build();
    // When
    Notification notificationResult = mapper.mapToNotification(notificationInfo);
    // Then
  ...
  void shouldMapAttachments() {
    ...
    NotificationInfo notificationInfo =
        NotificationInfo.builder()
            .notificationId(2L)
            .attachments(Arrays.asList(attachmentOne, attachmentTwo))
            .build();
    // When
    Notification notificationResult = mapper.mapToNotification(notificationInfo);
    // Then
  ...
  private static NotificationNote getNote(final long noteId, final String noteText) {
    return NotificationNote.builder()
        .noteId(noteId)
        .notificationId(2L)
  ...
  private static NotificationDocument getDocument(
      final long documentId, final String type, final String docDescription) {
  ...
  private static NotificationAttachment getAttachment(long attachmentId, String one) {
    return NotificationAttachment.builder()
        .attachmentId(attachmentId)
        .attachmentDescription(one)
        .build();
  }
}
